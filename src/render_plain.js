const renderPlain = (pkgs, durations) => {
  ...
  lines.push(
    ...Object.entries(durations).map(
      ([name, duration]) => `-> Request to ${name} took ${duration}ms`
    ),
    "------------------------------------------------------------"
  );
  ...
  // table header
  lines.push(
    `${"name".padEnd(50)}${"user".padEnd(10)}${"work".padEnd(
      10
    )}${"mark".padEnd(50)}`
  );
  ...
  for (let pkg of Object.values(pkgs).filter((pkg) => {
    ...
    lines.push(
      `${pkg.name.padEnd(50)}${(pkg.user || "").padEnd(
        10
      )}${pkg.work.kind.padEnd(10)}${pkg.mark
        .map(textMark)
        .join(", ")
        .padEnd(50)}`
    );
